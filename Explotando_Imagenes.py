...
import Criptografia, Fragmentar_unir, DatosImgs, Metadatos
...
class MainWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def comprobarCamposCifrado(self):
        if self.lineEdit.text() == "" or self.lineEdit_2.text() == "" or self.lineEdit_3.text() == "" or self.lineEdit_4.text() == "" or not self.radioButton.isChecked() and not self.radioButton_2.isChecked():
            ...
            try:
                ...
                extension = Metadatos.get_tipo(self, DOC)               # Obtiene la extensión.
                ...
                if self.radioButton.isChecked():
                    ...
                    self.listWidget.addItem("Ocultando información en imágenes empleando Metadatos...")
                    todir = DatosImgs.ocultar_en_imgs(self, fromdir, firma, extension, True)                            # Ocultamos la información en las imágenes.
                else:
                    fromdir = Fragmentar_unir.fragmentar(self, ENC_DOC.name, chunksize=200000) + '/'                    # Empleamos fragmentos de 200 KB.
                    self.listWidget.addItem("Ocultando información en imágenes empleando LSB...")
                    todir = DatosImgs.ocultar_en_imgs(self, fromdir, firma, extension, False)                           # Ocultamos la información en las imágenes.
                ...
                self.listWidget.addItem("¡HECHO! Información ocultada en imágenes correctamente. Ubicación: {}".format(todir))
    ...
    def comprobarCamposDescifrado(self):
        if self.lineEdit_5.text() == "" or self.lineEdit_6.text() == "" or not self.radioButton_3.isChecked() and not self.radioButton_4.isChecked():
            ...
            try:
                datos = DatosImgs.recuperar_de_imgs(self, dir_imagenes, self.radioButton_3.isChecked())
                ...
                DatosImgs.eliminarFragmentos(DIR_FRAGMENTOS)            # Eliminamos el directorio con los fragmentos
                extension = datos[1]
    ...
    def comprobarCamposFirma(self):
        if self.lineEdit_8.text() == "" or self.lineEdit_9.text() == "" or not self.radioButton_5.isChecked() and not self.radioButton_6.isChecked():
            ...
            try:
                # Extrae la firma de la primera imagen empleando Metadatos o LSB.
                firma = DatosImgs.obtener_firma(self, img, self.radioButton_6.isChecked())
                firma = DatosImgs.procesarFirma(self, firma)
    ...
    def abrirFichero(self, text, tipo_archivos):
        ...
        fich = QFileDialog.getOpenFileName(QFileDialog(), text, "", tipo_archivos, options=options)
    ...
    def abrirDirectorio(self):
        options = QFileDialog.Options()
        dir = QFileDialog.getExistingDirectory(QFileDialog(), "Seleccionar directorio imágenes", options=options)
